...
exports.eta = eta;
exports.etaHMS = etaHMS;
exports.etaYMDHMS = etaYMDHMS;
exports.etaDigital = etaDigital;
const jT_number_1 = __importDefault(require("./jT_number"));
...
function parseTime(str, options) {
    ...
    for (let query of timeQuery) {
        ...
        switch (op) {
            case "y":
                _parsed = time * 12 * 4 * 7 * 24 * 60 * 60 * 1000;
                break;
            case "mth":
                _parsed = time * 4 * 7 * 24 * 60 * 60 * 1000;
                break;
            case "w":
                _parsed = time * 7 * 24 * 60 * 60 * 1000;
                break;
            case "d":
                _parsed = time * 24 * 60 * 60 * 1000;
                break;
            case "h":
                _parsed = time * 60 * 60 * 1000;
                break;
            case "m":
                _parsed = time * 60 * 1000;
                break;
            case "s":
                _parsed = time * 1000;
                break;
    ...
    /* - - - - - { Return the Result } - - - - - */
    if (_options.fromNow)
        isNegative ? (sum = Date.now() - sum) : (sum = Date.now() + sum);
    if (_options.type === "s")
        sum = jT_number_1.default.msToSec(sum);
    if (!_options.fromNow && isNegative)
        sum = -sum;
    return sum;
}
...
function eta(unix, options) {
    ...
    let result = divisions.find((div, idx) => {
        if (difference < div.amount)
            return div;
        difference = Math.abs(difference / div.amount).toFixed(["milliseconds", "seconds", "minutes", "hours", "days"].includes(div.name) ? 0 : _options.decimalLimit);
    });
    if (!result)
        return null;
    // Grammar adjustment
...
function etaHMS(unix, options) {
    ...
    if (!difference)
        return "now";
    /* - - - - - { Preform Calculations } - - - - - */
    const seconds = jT_number_1.default.msToSec(difference);
...
exports.default = { parseTime, eta, etaHMS, etaYMDHMS, etaDigital };
//# sourceMappingURL=jT_date.js.map